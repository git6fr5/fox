...
public class Player : Controller {
    ...
    private bool CheckClimb() {
        ...
        for (int i = -1; i <= 1; i++) {
            Collider2D ground = Physics2D.OverlapCircle(transform.position + i * m_Height / 1.5f  * Vector3.up + direction * m_Width, GameRules.MovementPrecision, GameRules.ClimbCollisionLayer);
            if (ground != null) {
