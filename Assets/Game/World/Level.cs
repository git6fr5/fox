...
public class Level : MonoBehaviour {
    ...
    public void SetControls(List<LDtkTileData> controlData, Environment environment) {
        ...
        for (int i = 0; i < m_Entities.Count; i++) {
            if (m_Entities[i] != null && m_Entities[i].GetComponent<ElevatorPlatform>() != null) { elevators.Add(m_Entities[i].GetComponent<ElevatorPlatform>()); }
            if (m_Entities[i] != null && m_Entities[i].GetComponent<SwitchSpike>() != null) { switchSpikes.Add(m_Entities[i].GetComponent<SwitchSpike>()); }
            if (m_Entities[i] != null && m_Entities[i].GetComponent<Lever>() != null) { levers.Add(m_Entities[i].GetComponent<Lever>()); }
        }
