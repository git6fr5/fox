...
namespace Monet {
    ...
    public abstract class Enemy : Input {
        ...
        protected virtual void FixedUpdate() {
            if (!Game.Validate<Vector3>(m_Path)) {
                return;
            }
        ...
        protected Player Search(Vector3 origin, Vector3 target, float maxDistance) {
        ...
        private void BaseAction() {
            m_FacingDirection = m_Direction.x != 0f ?  Mathf.Sign(m_Direction.x) : m_FacingDirection;
            m_AttackDirection = new Vector2(m_FacingDirection, 0f);
        }
